      ...
      <div v-else class="table-wrapper">
        <el-table :data="tableData" border stripe style="width: 100%">
          ...
          <el-table-column label="体检日期" min-width="160" align="center">
            <template #default="scope">{{ formatDateTime(scope.row.checkDate || scope.row.check_date) }}</template>
          </el-table-column>
          ...
          <el-table-column label="心率" width="100" align="center">
            <template #default="scope">{{ scope.row.heartRate || scope.row.heart_rate || '-' }}</template>
          </el-table-column>
          <el-table-column label="血压" width="120" align="center">
            <template #default="scope">{{ scope.row.bloodPressure || scope.row.blood_pressure || '-' }}</template>
          </el-table-column>
          <el-table-column prop="result" label="体检结果" min-width="180" show-overflow-tooltip />
          <el-table-column label="医生建议" min-width="220" show-overflow-tooltip>
            <template #default="scope">{{ scope.row.suggestion || scope.row.advice || '-' }}</template>
          </el-table-column>
        </el-table>
      </div>
    </el-card>
